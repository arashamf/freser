...
//		return 0; // Return false 
	...
	uint32_t p = 2500; /* A little delay */
	while(p>0)
		p--;
...
*/
	...
	SSD1306_WRITECOMMAND(0xA1); //--set segment re-map 0 to 127  0xA1
	...
	SSD1306_WRITECOMMAND(0x02); //0x20,0.77xVcc     00XXXX00b
...
void SSD1306_UpdateScreen(void) {
	...
	{    
		...
		/* Write multi data */
		ssd1306_I2C_WriteMulti(SSD1306_I2C_ADDR, 0x40, &SSD1306_Buffer[SSD1306_WIDTH * m-4], SSD1306_WIDTH);
	}
}
...
char SSD1306_Putc(char ch, FontDef_t* Font, SSD1306_COLOR_t color) 
{
	...
	if (SSD1306_WIDTH <= (SSD1306.CurrentX + Font->FontWidth) ||
		SSD1306_HEIGHT <= (SSD1306.CurrentY + Font->FontHeight)) // Check available space in LCD //
		{return 0;}     /* Error */
	/* Go through font */
	for (i = 0; i < Font->FontHeight; i++) 
	{
		...
		for (j = 0; j < Font->FontWidth; j++) 
		{
			...
			else {SSD1306_DrawPixel(SSD1306.CurrentX + j, (SSD1306.CurrentY + i), (SSD1306_COLOR_t)!color);}
		}
	}	
...
void SSD1306_DrawLine(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, SSD1306_COLOR_t c) 
{
	...
	if (dy == 0) 
	{
		...
		for (i = x0; i <= x1; i++) 		// Horizontal line 
