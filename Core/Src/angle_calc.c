...
{	
	...
	handle->step_shaft_degree = handle->AngleTeethInSec/(3600); //угол в градусах
	tmp = handle->AngleTeethInSec % 3600; //остаток с минутами и секундами
	handle->step_shaft_minute = tmp/60;
	handle->step_shaft_second = tmp % 60;
}
...
void teeth_angle_to_EEPROMbuf (milling_data_t * handle, uint8_t * EEPROM_buffer)
{
	...
	*(EEPROM_buffer+14) = (uint8_t)(handle->AngleTeethInSec >> 8); //сначала старшая часть градуса
	*(EEPROM_buffer+15) = (uint8_t)(handle->AngleTeethInSec); 	//младшая часть градуса		
}
